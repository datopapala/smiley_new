...
$text[0] 	= "პრეტენზიები პროდუქტების  მიხედვით";
	...
	case  1:
		$result = mysql_query("	SELECT 	incomming_call.`production_id` AS c_name,
										...
										CONCAT(ROUND(COUNT(*)/(
											...
											WHERE incomming_call.information_sub_category_id=37  AND production_category_id='$category' and DATE(`incomming_call`.`date`) >= '$start' AND DATE(`incomming_call`.`date`) <= '$end'
											GROUP BY c_name
													)*100,2),'%')
