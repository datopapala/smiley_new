...
$text[0] 	= "ინტერესი ბრენდების  მიხედვით";
	...
	case  1:
		$result = mysql_query("	SELECT 	incomming_call.`production_id` AS c_name,
										...
										CONCAT(ROUND(COUNT(*)/(
											...
											WHERE production_type=2  AND production_brand_id='$category' and DATE(`incomming_call`.`date`) >= '$start' AND DATE(`incomming_call`.`date`) <= '$end'
											GROUP BY c_name
													)*100,2),'%')
