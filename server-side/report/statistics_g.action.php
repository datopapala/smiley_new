...
$text[0] 	= "ინტერესი პროდუქტის კატეგორიის მიხედვით";
...
switch ($done){
	case  1:
		$result = mysql_query("	SELECT 	incomming_call.`production_id` AS c_name,
										...
										CONCAT(ROUND(COUNT(*)/(
											...
											WHERE production_type=2  AND production_category_id='$category' and DATE(`incomming_call`.`date`) >= '$start' AND DATE(`incomming_call`.`date`) <= '$end'
											GROUP BY c_name
													)*100,2),'%')
	...
	default:
		$result = mysql_query("SELECT 	incomming_call.`production_category_id` AS c_name,
										...
										CONCAT(ROUND(COUNT(*)/(
											SELECT COUNT(*) FROM incomming_call
											WHERE incomming_call.production_type=2 and DATE(`incomming_call`.`date`) >= '$start' AND DATE(`incomming_call`.`date`) <= '$end'
												)*100,2),'%')
							FROM 		incomming_call
