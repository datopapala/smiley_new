...
$text[0] 	= "ჭყარო პროდუქტების  მიხედვით";
...
switch ($done){
	case  1:
		$result = mysql_query("	SELECT 	incomming_call.`production_id` AS c_name,
										...
										CONCAT(ROUND(COUNT(*)/(
											...
											JOIN surce ON surce.id =incomming_call.id
											WHERE surce.`name`='$category' and DATE(`incomming_call`.`date`) >= '$start' AND DATE(`incomming_call`.`date`) <= '$end'
											GROUP BY c_name
													)*100,2),'%')
	...
	case 3:
		...
		break; */
	default:
		$result = mysql_query("SELECT 	surce.`name` AS c_name,
										COUNT(*),
										CONCAT(ROUND(COUNT(*)/(
											SELECT COUNT(*) FROM incomming_call
											JOIN surce ON surce.id =incomming_call.id
